...
for i in range(len(numbers)):
    for j in range(i + 1, len(numbers)):
        equation = Equation(make_tree(numbers[i])[0])
        equation.add(make_tree(numbers[j])[0])
        equation.reduce()
        m = equation.magnitude()
        if (biggest is None) or (m > biggest):
            biggest = m
            big_left = i
            big_right = j

        equation = Equation(make_tree(numbers[j])[0])
        equation.add(make_tree(numbers[i])[0])
        equation.reduce()
        m = equation.magnitude()
        if (biggest is None) or (m > biggest):
            biggest = m
...
print(biggest)
print(numbers[big_left])
print(numbers[big_right])
